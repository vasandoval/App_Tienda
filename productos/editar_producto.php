...
$id = intval($_GET['id'] ?? 0);
if ($id === 0) {
    header("Location: listar_productos.php?error=Producto+no+válido");
    exit;
}
...
$producto = $stmt->fetch();
if (!$producto) {
    header("Location: listar_productos.php?error=Producto+no+encontrado");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Código duplicado (excluyendo el mismo)
    if (empty($errores)) {
    ...
    if (empty($errores)) {
        $stmt = $pdo->prepare("UPDATE productos SET
            codigo = ?, nombre = ?, peso = ?,
            cantidad_almacenada = ?, id_categoria = ?,
            id_empaque = ?, precio_unitario = ?
            WHERE id_producto = ?");
        $stmt->execute([$codigo, $nombre, $peso, $cantidad, $id_categoria, $id_empaque, $precio, $id]);
        header("Location: listar_productos.php?msg=Producto+actualizado+correctamente");
        exit;
    }

    // Actualizar datos en pantalla con lo que envió
...
<div class="main-content">
  ...
  <div class="page-header">
    <div>
      <h1 class="page-title">
        <span class="title-icon" style="background:#FEF3C7;">✏️</span>
        Editar Producto
      </h1>
      <p class="page-subtitle">Modificar datos de: <strong><?= htmlspecialchars($producto['nombre']) ?></strong></p>
    </div>
    ...
    <div class="card-body">
      <form method="POST">
        <div class="form-grid">
          ...
          <div class="form-group">
            <label for="id_empaque">Empaque <span class="required">*</span></label>
