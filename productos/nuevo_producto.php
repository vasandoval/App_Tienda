...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $pdo->prepare("INSERT INTO productos
            (codigo, nombre, peso, cantidad_almacenada, id_categoria, id_empaque, precio_unitario)
            VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$codigo, $nombre, $peso, $cantidad, $id_categoria, $id_empaque, $precio]);
        header("Location: listar_productos.php?msg=Producto+registrado+correctamente");
        exit;
    }
}

// Mapa IVA para mostrar info
$iva_map = [];
foreach ($categorias as $c) {
    $iva_map[$c['id_categoria']] = $c['impuesto'];
}
?>
<!DOCTYPE html>
<html lang="es">
...
<div class="main-content">
  ...
  <div class="page-header">
    ...
    <a href="listar_productos.php" class="btn btn-outline">← Volver</a>
  </div>
    ...
    <div class="card-body">
      <form method="POST">
        <div class="form-grid">
          ...
          <div class="form-group">
            <label for="id_empaque">Tipo de empaque <span class="required">*</span></label>
          ...
          <div class="form-group">
            <label for="peso">Peso (gramos) <span class="required">*</span></label>
          ...
          <div class="form-group">
            <label for="cantidad_almacenada">Cantidad en stock <span class="required">*</span></label>
          ...
          <div class="form-group">
            <label for="precio_unitario">Precio unitario ($) <span class="required">*</span></label>
          ...
          <div class="form-group">
            ...
            <div id="iva-info" class="alert alert-info" style="margin:0;">
              Selecciona una categoría para ver el IVA
            </div>
          </div>
        ...
        <div style="margin-top:1.5rem; display:flex; gap:0.8rem;">
          ...
          <a href="listar_productos.php" class="btn btn-outline btn-lg">Cancelar</a>
        </div>
      </form>
    </div>
  </div>
...
<script>
// Mostrar IVA según categoría seleccionada
document.getElementById('id_categoria').addEventListener('change', function() {
  ...
  if (iva !== undefined) {
    ...
    box.className = 'alert ' + (iva > 0 ? 'alert-warning' : 'alert-success');
    box.style.margin = '0';
  }
});
</script>
